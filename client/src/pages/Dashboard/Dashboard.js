...
const Dashboard = ({ onTitleChange }) => {
  ...
  const [text, setText] = useState([
    { Object: 'Total PPE Incidents', Count: 26 },
  ...
  useEffect(() => {
    if (localStorage.monthlocaldata) {
  ...
  useEffect(() => {
    if (!localStorage.from_year) {
      localStorage.from_year = 2023;
      setfromyear(Number(localStorage.from_year));
    } else {
      setfromyear(Number(localStorage.from_year));
    }
    setisLoading(true);
  ...
  useEffect(() => {
    setfromyear(Number(localStorage.from_year) || '');
    setToYear(Number(localStorage.to_year) || '');
    setFromMonth(new Date(Number(localStorage.year) || 2023, Number(localStorage.from_month)-1 || 0));
    setToMonth(new Date(Number(localStorage.year_1) || 2023, Number(localStorage.to_month)-1 || 11));
    setFromDate(localStorage.from_date || '');
    setToDate(localStorage.to_date || '');
  }, []);
...
    localStorage.from_month = month_1;
  ...
  const fetchdateData = () => {
    var date_id = document.getElementById('daily_id');
    var value = date_id.value;
    console.log(value + 'something' + date_id.value);
    ...
    axios.post('http://localhost:8083/api/dashboard/dailydata', data_set_date).then((res) => {
      if (res.data.length === 0) {
        // Handle no data case
      } else {
          ...
          <Grid container spacing={3} sx={{ marginTop: '20px' }}>
            <Grid item xs={12} md={6}>
              <Paper className="paper">
                <Typography variant="h6" sx={{ paddingTop: '10px', display: 'inline' }} className="text">
                  Yearly PPE Incidents
                </Typography>
                <div className="container">
                  ...
                  <div>
                    <button onClick={() => fetchyearData()}>REFRESH</button>
                  </div>
                </div>
            ...
            <Grid item xs={12} md={6}>
              <Paper className="paper">
                ...
                <div className="container">
                  ...
                  <Form.Select aria-label="Default select example" size="sm" className="datepicker" id="monthly_id" >
                    <option value="BSA-LH">Line 1</option>
                    <option value="BSA-LH">Line 2</option>
                    <option value="BSA-LH">Line 3</option>
                  </Form.Select>
                  <div>
                    <button onClick={() => fetchmonthlyData()}>REFRESH</button>
                  </div>
                </div>
